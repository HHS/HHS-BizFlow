...
<head>
	...
	<title>EWITS 2.0 Job Aid Home</title>
...
<div class="container">
	<nav aria-label="breadcrumb">
	  <ol class="breadcrumb">
		...
		<li class="breadcrumb-item active" aria-current="page"><a href="#">EWITS 2.0 Job Aid Materials</a></li>
	  </ol>
	</nav>
</div>
...
for (int i=0; i<list.length; i++) {
	...
	if (!"JSP".equalsIgnoreCase(getFileExtension(fileName))) {
%>
			<tr>
			  <th scope="row"><%=++idx%></th>
			  ...
			  <td>
				...
				<button type="button" class="btn btn-secondary fas fa-file-download" data-toggle="tooltip" data-placement="top" title="Download file"onclick="download(<%=i%>);"></button>
			  </td>
			</tr>
<%
	}
...
<div class="container">
    <form action="uploader_action.jsp" method="post" enctype="multipart/form-data">
		<br/>
        <div>
			<input type="file" name="file1" size="50" accept=".pdf,.ppt,.pptx,.doc,.docx" />
		</div>
        <div>
			<i class="fas fa-upload"></i> Click the Choose File button to upload your Job Aid document file (pdf, ppt, pptx, doc, docx):
		</div>
        <hr/>
        <div>
