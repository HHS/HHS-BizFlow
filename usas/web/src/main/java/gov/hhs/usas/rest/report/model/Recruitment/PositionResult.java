...
public class PositionResult
{
	...
	@XmlTransient
	private List<Integer> gradeList;//Type Integer to enable sorting
	...
	public String getGrades()
	{
		...
		Collections.sort(this.gradeList);//sort the list of Grades in ascending order for better display
		for (int i = 0; i < this.gradeList.size(); i++) {
			String grade = this.gradeList.get(i).toString();
			if (i > 0) {
				gradesStr.append("," + grade);
			} else {
				gradesStr.append(grade);
			}
			/*if (i > 0) {
	...
	/* public void setGrades(List<String> grades)
  {
	...
	{	 
		if (grade.trim().length() > 0) {
			Integer gradeInt = Integer.parseInt(grade.trim());
			if((!this.gradeList.contains(gradeInt)))
				this.gradeList.add(gradeInt);
		}
