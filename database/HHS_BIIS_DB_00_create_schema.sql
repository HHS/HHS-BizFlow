...
CREATE USER HISTDBA IDENTIFIED BY <replace_with_password>
	...
	QUOTA UNLIMITED ON HHS_BIIS_TS
;
-- EHRP through BIIS view schema
CREATE USER EHRP IDENTIFIED BY <replace_with_password>
	DEFAULT TABLESPACE HHS_BIIS_TS
	QUOTA UNLIMITED ON HHS_BIIS_TS
;
...
CREATE ROLE HHS_BIIS_EHRP_OWNER_ROLE;
...
GRANT CONNECT, RESOURCE, HHS_BIIS_EHRP_OWNER_ROLE TO EHRP;
...
-- grant owner database privileges to BIIS owner role
...
-- grant owner database privileges to EHRP owner role
GRANT ALTER SESSION, CREATE CLUSTER, CREATE DATABASE LINK
		, CREATE SEQUENCE, CREATE SESSION, CREATE SYNONYM, CREATE TABLE
		, CREATE VIEW, CREATE PROCEDURE
	TO HHS_BIIS_EHRP_OWNER_ROLE
;
