...
--------------------------------------------------------
--  DDL for Procedure SP_CDC_JR_STG2FNL
--------------------------------------------------------
CREATE OR REPLACE PROCEDURE HHS_HR.SP_CDC_JR_STG2FNL
IS
	V_STG_CNT                   NUMBER;
	V_ERRCODE                   NUMBER(10);
	V_ERRMSG                    VARCHAR2(512);
	...
	IS
		SELECT REQUEST_NUMBER
			...
			, HIRING_AUTHORITY
			, REQUEST_CREATE_DATE
		FROM HHS_HR.DSS_CDC_JR_RLTNSHP_STG;

		TYPE TYPE_JR_STG IS TABLE OF CUR_JR_STG%ROWTYPE
			INDEX BY PLS_INTEGER;

		V_TBL_JR_STG TYPE_JR_STG;
BEGIN
	BEGIN
		SELECT COUNT(*)
		INTO V_STG_CNT
		...
		IF V_STG_CNT > 0 THEN
			EXECUTE IMMEDIATE 'TRUNCATE TABLE DSS_CDC_JR_RLTNSHP';
			OPEN CUR_JR_STG;
			LOOP
				FETCH CUR_JR_STG
				BULK COLLECT INTO V_TBL_JR_STG
				LIMIT 1000;
				
				FORALL i IN V_TBL_JR_STG.FIRST..V_TBL_JR_STG.LAST SAVE EXCEPTIONS
					INSERT INTO DSS_CDC_JR_RLTNSHP VALUES V_TBL_JR_STG(i);
				COMMIT;
				
				EXIT WHEN CUR_JR_STG%NOTFOUND;
			END LOOP;
			CLOSE CUR_JR_STG;
		END IF;
	EXCEPTION
		WHEN OTHERS THEN
			...
			V_ERRCODE := SQLCODE;
			V_ERRMSG := SQLERRM;
		--DBMS_OUTPUT.PUT_LINE('ERROR occurred while executing SP_CDC_JR_STG2FNL -------------------');
		--DBMS_OUTPUT.PUT_LINE('Error code    = ' || V_ERRCODE);
		--DBMS_OUTPUT.PUT_LINE('Error message = ' || V_ERRMSG);
	END;
	--DBMS_OUTPUT.PUT_LINE('SP_CDC_JR_STG2FNL - END ==========================');
EXCEPTION
	WHEN OTHERS THEN
		SP_ERROR_LOG();
		V_ERRCODE := SQLCODE;
