...
IS
	...
	WHERE EFFDT >= TO_DATE('01/01/2016','MM/DD/YYYY');
...
--------------------------------------------------------
--CURSOR: CUR_EMP_UPD
--DESCRIPTION: Fetches max WGI due date and Service
-- Computation Date from EHRP PS_GVT_EMPLOYMENT 
--------------------------------------------------------
CURSOR CUR_EMP_UPD
IS
	SELECT 
	  	EMPLID
  		, MAX(GVT_WGI_DUE_DATE) AS WGI_DUE_DATE
  		, MAX(GVT_SCD_RETIRE) AS SCD
	FROM EHRP.PS_GVT_EMPLOYMENT@BIIS_DBLINK E
	WHERE EFFDT  >= TO_DATE('01/01/2016','MM/DD/YYYY')
	AND EXISTS (SELECT 1 FROM EMPLOYEE_LOOKUP
                 WHERE EMPLID = E.EMPLID)
	GROUP BY EMPLID;
	
	TYPE TYP_EMP_UPD IS TABLE OF CUR_EMP_UPD%ROWTYPE
		INDEX BY PLS_INTEGER;
		
	TYPE TYP_EMP_CACHE IS TABLE OF CUR_EMP_UPD%ROWTYPE
		INDEX BY VARCHAR2(50);
		
--------------------------------------------------------
--CURSOR: CUR_EMPLID
--DESCRIPTION: Fetches the employee id from the 
...
IS
	...
	TYPE TYP_EMPLID IS TABLE OF CUR_EMPLID%ROWTYPE
		INDEX BY PLS_INTEGER;
...
PROCEDURE UPDATE_EMPLOYEE_LOOKUP
AS
	V_EMP_UPD	TYP_EMP_UPD;
	V_EMP_CACHE	TYP_EMP_CACHE;
	V_EMPLID	TYP_EMPLID;
BEGIN
	
	OPEN CUR_EMP_UPD;
	FETCH CUR_EMP_UPD
	BULK COLLECT INTO V_EMP_UPD;
	CLOSE CUR_EMP_UPD;

	
	OPEN CUR_EMPLID;
	FETCH CUR_EMPLID
	BULK COLLECT INTO V_EMPLID;
	CLOSE CUR_EMPLID;

	
	IF V_EMP_UPD.COUNT > 0 THEN
		FOR i IN V_EMP_UPD.FIRST .. V_EMP_UPD.LAST LOOP
			IF V_EMP_UPD(i).EMPLID IS NOT NULL THEN
				V_EMP_CACHE(V_EMP_UPD(i).EMPLID).EMPLID 		:= V_EMP_UPD(i).EMPLID;
				V_EMP_CACHE(V_EMP_UPD(i).EMPLID).WGI_DUE_DATE 	:= V_EMP_UPD(i).WGI_DUE_DATE;
				V_EMP_CACHE(V_EMP_UPD(i).EMPLID).SCD 			:= V_EMP_UPD(i).SCD;
			END IF;
		END LOOP;
	END IF;
	
	IF V_EMPLID.COUNT > 0 THEN
		FOR j IN V_EMPLID.FIRST .. V_EMPLID.LAST LOOP
			IF V_EMP_CACHE.EXISTS(V_EMPLID(j).EMPLID) THEN
				UPDATE HHS_HR.EMPLOYEE_LOOKUP
				SET WGI_DUE_DATE = V_EMP_CACHE(V_EMPLID(j).EMPLID).WGI_DUE_DATE,
					SCD = V_EMP_CACHE(V_EMPLID(j).EMPLID).SCD
				WHERE EMPLID = V_EMPLID(j).EMPLID;
			END IF;
		END LOOP;
		COMMIT;
	END IF;
